...
function getTrip(range, tripText) {
  try {
    ...
    if (tripData[1] > 0) {
      var message =
        '<b>🛵 Trip ' + tripText + ' Details</b>\n' +
        '\n<code>----------:TRIP INFO:---------</code>' +
        '\nDate: ' + formatDateTime(tripData[25]) +
        '\nID: ' + (tripData[0]) +
        '\n\n<code>----------:DISTANCE:----------</code>' +
        '\nDistance: ' + (tripData[1]).toFixed(1) + ' Km' +
        getRideModeData(tripData) +
        '\nBraking: ' + (tripData[10]).toFixed(1) + ' Km' +
        '\nCoasting: ' + (tripData[14]).toFixed(1) + ' Km' +
        '\n\n<code>-------:OTHER METRICS:--------</code>' +
        '\nDuration: ' + (tripData[2] * 24 * 60).toFixed(0) + ' mins' +
        '\nEfficiency: ' + (tripData[3]).toFixed(1) + ' Wh/km' +
        '\nFuel Savings: ₹' + (tripData[24]).toFixed(2) +
        '\nRange: ' + (tripData[5]).toFixed(1) + ' Km' +
        '\nTop Speed: ' + (tripData[4]).toFixed(1) + ' Km/h' +
        '\nAvg Speed: ' + (tripData[6]).toFixed(1) + ' Km/h' +
        '\nSOC: ' + (tripData[8] * 100).toFixed(2) + '%' +
        '\nSOC/Km: ' + (tripData[9] * 100).toFixed(2) + '%' +
        '\nHorn Count: ' + (tripData[23]).toFixed(0)
    }
  } catch (error) {
